...
// set all elements in 3D array as -1 so that any value 0 and above will be recognized as a registered cube  
  ...
  int[] adjacentCubesXYZ(PVector xyz)
  {
    ...
    for (int i = (int)xyz.x - clusterRadius; i <= (int)xyz.x + clusterRadius; i++)
    {
      for (int j = (int)xyz.y - clusterRadius; j <= (int)xyz.y + clusterRadius; j++)
      {
        for (int k = (int)xyz.z - clusterRadius; k <= (int)xyz.z + clusterRadius; k++)
        {
